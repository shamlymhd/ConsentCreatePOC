...
public class ConsentMgtCommonDBQueries {
    ...
    public String getGetConsentStatusAuditRecordsPreparedStatement() {
        ...
        return "SELECT * FROM OB_CONSENT_STATUS_AUDIT WHERE CONSENT_ID = COALESCE(?, CONSENT_ID) AND CURRENT_STATUS =" +
                " COALESCE(?, CURRENT_STATUS) AND ACTION_BY = COALESCE(?, ACTION_BY) AND STATUS_AUDIT_ID = COALESCE " +
                "(?, STATUS_AUDIT_ID) AND ACTION_TIME >= COALESCE(?, ACTION_TIME) AND ACTION_TIME <= COALESCE(?, " +
                "ACTION_TIME)";
    }
    ...
    public String getUpdateConsentValidityTimePreparedStatement() {
        ...
        return "UPDATE OB_CONSENT SET VALIDITY_TIME = ? WHERE CONSENT_ID = ?";
    }
